...
def WriteLog(Summary,Raiser,Status): pass
def ShellOut(Message, Type): pass
def GetVideoList(url): pass


def WriteLog(Summary, Raiser, Detail="None"):
    ...
    try:
        ...
        print(f"{SHELL_ERROR}写入日志文件失败: {str(e)}。\n试图写入日志的内容如下，请将其提交开发者:\n{log_entry}{SHELL_ERROR}")
        
def ShellOut(Message, Type=MessageType.TRIVIAL):
    match Type:
        case MessageType.TRIVIAL:
            print(Message)
        ...
        case _:  # Default case
            print(Message)
            ShellOut("程序发生内部错误，但仍在继续运行。请将日志提交开发者。",MessageType.WARNING)
            WriteLog ("Wrong Parameter Passed", "ShellOut", f"Parameters: \n Type={str(MessageType)}\n Message={Message}")
...
def GetVideoList(url=URL):
    ...
    [chrome_options.add_argument(arg) for arg in chrome_config['arguments']]
    [chrome_options.add_experimental_option(key, value) for key, value in chrome_config['experimental_options'].items()]
    
    try:
        ShellOut("B站限制未登录用户浏览内容，请您在即将打开的网页中登录，以便程序获取视频内容。登陆后请回到本界面。\n 程序将于5秒后打开Chrome浏览器登录界面。\n 登录成功后，请输入'y'继续",Message.INFO)
        ...
        user_input = input(f"{SHELL_ACTION}您是否已成功登录B站？(成功请输入y):{SHELL_RESET}").lower()
        if user_input == 'y':
            print("登录确认，继续执行...")
        else:
            print("退出程序...")
